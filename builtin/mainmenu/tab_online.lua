...
local function get_sorted_servers()
    ...
    -- Add five placeholder servers that are always online
    local placeholder_servers = {
        {
            name = "WoodBoat Network",
            address = "157.173.202.134",
            port = 19190,
            description = "Welcome to woodboat network an amazing minecraft lifeboat remake",
            clients = 111,
            clients_max = 100,
            proto_min = 25, -- Assuming compatible protocol versions
            proto_max = 40,
            is_compatible = true,
            ping = 0, -- Good ping
            mods = {"nodata", "nodata"},
            clients_list = {"nodata", "nodata"},
            gameid = "minetest",
            is_favorite = false,
            url = "https://null" -- Added URL for consistency
        },
        {
            name = "PocketBoat PE",
            address = "157.173.202.134",
            port = 30000,
            description = "Pocketboat PE minecraft server for minetest",
            clients = 111,
            clients_max = 100,
            proto_min = 25,
            proto_max = 40,
        ...
        -- NEW: Added two new placeholder servers
        {
            name = "Eriks Kingdom",
            address = "157.173.202.134",
            port = 19134,
            description = "Eriks kingdom a server rules by a player named erik",
            clients = 50,
            clients_max = 200,
            proto_min = 25,
            proto_max = 40,
            is_compatible = true,
            ping = 0,
            mods = {"skyblock", "adventure"},
            clients_list = {"player1", "player2"},
            gameid = "minetest",
            is_favorite = false,
            url = "https://null"
        },
        {
            name = "Minetest Alpha v0.2.3",
            address = "157.173.202.134",
            port = 51337,
            description = "T͐͐w̿̽ĭ̓s̊͝ẗ́͝e͋̇d͐͝ ̌̒w̋̈́o͒̊ṙ̓ḋ͝s̒̓ ̐͠c̐͝r̋̇a̒̓w͝͝l̽̈́ ̋͐f̐͝r̋̈́o̐̈́m̐͝ ̋̈́t̒̽h̐̒e̽̽ ̆͝v̐͝o̐͝i̐͝d̒͝.̊̒.̒͝.̆͝",
            clients = 75,
            clients_max = 150,
            proto_min = 25,
            proto_max = 40,
            is_compatible = true,
            ping = 0,
            mods = {"creative", "building"},
            clients_list = {"builder1", "builder2"},
            gameid = "minetest",
            is_favorite = false,
            url = "https://alpha.etherdeck.org"
        }
    }
    -- Insert placeholder servers into featured servers
    for _, server in ipairs(placeholder_servers) do
        table.insert(servers.featured, server)
    end
...
local function find_selected_server()
    ...
    -- NEW: Check placeholder servers if no match found in serverlistmgr or favorites
    local placeholder_servers = {
        {
            name = "WoodBoat Network",
            address = "157.173.202.134",
        ...
        },
        {
            ...
            address = "157.173.202.134",
            port = 19134,
            description = "SkyCraft Hub: A Minetest adventure server with skyblock challenges",
            clients = 50,
            clients_max = 200,
            proto_min = 25,
            proto_max = 40,
...
local function get_formspec(tabview, name, tabdata)
    ...
    if selected_server then
        ...
        -- NEW: Always enable view buttons for placeholder servers
        local is_placeholder = selected_server.name == "WoodBoat Network" or
                              selected_server.name == "PocketBoat PE" or
                              selected_server.name == "BackToAlpha" or
                              selected_server.name == "SkyCraft Hub" or
                              selected_server.name == "PixelCraft Realms"
        -- Mods button
        local mods = selected_server.mods
        local can_view_mods = mods and #mods > 0 or is_placeholder
        if can_view_mods then
            local tooltip = ""
            if selected_server.gameid then
                tooltip = fgettext("Game: $1", selected_server.gameid) .. "\n"
            end
            tooltip = tooltip .. fgettext("Number of mods: $1", mods and #mods or 0)
            retval = retval ..
        ...
        local can_view_clients_list = (clients_list and #clients_list > 0) or is_placeholder
        if can_view_clients_list then
            ...
            local max_clients = 5
            if clients_list and #clients_list > max_clients then
                retval = retval .. "tooltip[btn_view_clients;" ..
                    fgettext("Clients:\n$1", table.concat(clients_list, "\n", 1, max_clients)) .. "\n..." .. "]"
            elseif clients_list then
                retval = retval .. "tooltip[btn_view_clients;" ..
                    fgettext("Clients:\n$1", table.concat(clients_list, "\n")) .. "]"
            else
                retval = retval .. "tooltip[btn_view_clients;" .. fgettext("View players") .. "]"
            end
        ...
        -- URL button
        local has_url = selected_server.url or is_placeholder
        if has_url then
...
local function main_button_handler(tabview, fields, name, tabdata)
    ...
    if fields.btn_server_url then
        local selected_server = find_selected_server()
        if selected_server and selected_server.url then
            core.open_url_dialog(selected_server.url)
        end
        return true
    end
    if fields.btn_view_clients then
        local selected_server = find_selected_server()
        if selected_server and (selected_server.clients_list or
            (selected_server.name == "WoodBoat Network" or
             selected_server.name == "PocketBoat PE" or
             selected_server.name == "BackToAlpha" or
             selected_server.name == "SkyCraft Hub" or
             selected_server.name == "PixelCraft Realms")) then
            local dlg = create_clientslist_dialog(selected_server)
    ...
    if fields.btn_view_mods then
        local selected_server = find_selected_server()
        if selected_server and (selected_server.mods or
            (selected_server.name == "WoodBoat Network" or
             selected_server.name == "PocketBoat PE" or
             selected_server.name == "BackToAlpha" or
             selected_server.name == "SkyCraft Hub" or
             selected_server.name == "PixelCraft Realms")) then
            local dlg = create_server_list_mods_dialog(selected_server)
            dlg:set_parent(tabview)
            tabview:hide()
            dlg:show()
        end
